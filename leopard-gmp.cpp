...
void *parallel_lsh(void *args) {
    ...
    /*
    lsh(0,wc*hc-1,codeCam,matchCam,maskCam,wc,hc,codeProj,matchProj,maskProj,wp,hp,sp?1:-1,mix);
    lsh(wc*hc-1,0,codeCam,matchCam,maskCam,wc,hc,codeProj,matchProj,maskProj,wp,hp,sp?1:-1,mix);
    lsh(0,wp*hp-1,codeProj,matchProj,maskProj,wp,hp,codeCam,matchCam,maskCam,wc,hc,sp?0:-1,mix);
    lsh(wp*hp-1,0,codeProj,matchProj,maskProj,wp,hp,codeCam,matchCam,maskCam,wc,hc,sp?0:-1,mix);
    */
}
#endif
...
int leopard::doLsh(int sp, unsigned char mix) {
	...
	bminfo bm[nv*4]; // on choisi tout de suite les bits pour 4 lsh
    choisiLshBits(bm,nv*4);
#ifdef USE_THREADS
    pthread_barrier_init(&barrier, NULL, nbT);
...
void leopard::choisiLshBits(bminfo *bm,int nb) {
...
#ifdef USE_GMP
int leopard::lsh(int start,int stop,mpz_t *codeA,minfo *matchA,unsigned char *maskA,int wa,int ha,

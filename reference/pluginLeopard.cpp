...
//
// Attention!
// This is only valid for the Apple display...
//
#define SCREEN_WIDTH	1920
#define SCREEN_HEIGHT	1200
#define PIXELS_PER_MM	38.76

pluginLeopard::pluginLeopard() {
	// ports
	ports["incam"]=Q_IN_CAM;
	ports["inproj"]=Q_IN_PROJ;
	ports["out"]=Q_OUT;
	ports["log"]=Q_LOG;
	ports["thrash"]=Q_TRASH; // input images are sent there
	ports["recycle"]=Q_RECYCLE; // input images are sent there
...
pluginLeopard::~pluginLeopard() { log << "deleted"<<warn; }

void pluginLeopard::init() {
...
int pluginLeopard::cost(unsigned char *a,unsigned char *b,int sz) {
	int i,k=0;
...
void pluginLeopard::match2image(blob *lut,minfo *match,int w,int h,int ww,int hh,int maxcost) {
...
//
void pluginLeopard::match2opencv(minfo *match,int w,int h,int ww,int hh,int nbpoints,int maxcost,int iterations) {
	point *T;
...
int pluginLeopard::sort(unsigned char *codeA,minfo *matchA,int wa,int ha,
...
int pluginLeopard::lsh(int dir,unsigned char *codeA,minfo *matchA,int wa,int ha,
			      unsigned char *codeB,minfo *matchB,int wb,int hb) {
...
int pluginLeopard::heuristique( unsigned char *codeA,minfo *matchA,int wa,int ha,
						 unsigned char *codeB,minfo *matchB,int wb,int hb) {
...
int pluginLeopard::roundtrip( unsigned char *codeA,minfo *matchA,int wa,int ha,
...
// /set/screen w h ppm
//
bool pluginLeopard::decode(const osc::ReceivedMessage &m) {
	log << "decoding " << m << warn;
